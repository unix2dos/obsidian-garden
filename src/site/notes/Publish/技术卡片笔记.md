---
{"aliases":[],"tags":[],"title":"技术卡片笔记","date":"2025-06-01T07:55:23+08:00","date_modify":"2026-05-08T23:48:18+08:00","dg-publish":true,"permalink":"/Publish/技术卡片笔记/","dgPassFrontmatter":true,"created":"2025-06-01T07:55:23+08:00","updated":"2026-05-08T23:48:18+08:00"}
---


# 2026-05-08 Claude 养号和退款申请

1. 指纹浏览器
2. 家庭带宽 ip
3. 同电脑不要多号存在
4. 不要同时超过两个 session 使用

My account was banned by Anthropic for no reason.

- apple: [<https://reportaproblem.apple.com/?s=6>](https://reportaproblem.apple.com/status?s=6)
- google play: <https://support.google.com/googleplay/workflow/9813244>
- google play: <https://support.google.com/googleplay/workflow/10728048>

# 2026-05-08 Chrome 插件管理
...
右键菜单 — tmux 默认右键就是 display-menu，你试试在 pane 上右键就能看到

状态栏点击 — 点击状态栏上的 session/window 名弹出切换菜单

快捷操作面板 — 就像刚加的，把常用操作收在一个入口

确认危险操作 — 比如 kill 之前弹个菜单选 " 确认/取消 "

本质上就是一个轻量级的选择器，适合 " 选一个就走 " 的场景。复杂交互（搜索、预览）还是得用 popup + fzf。

## Tmux Mark

功能：给某个 pane 打个 " 标记 "，之后可以用 {marked} 引用它，常用于：

标记当前 Pane

Prefix + m → select-pane -m # 边框会变虚线表示已标记

用标记做操作

swap-pane -t {marked} # 把当前 pane 和标记的 pane 交换位置

join-pane -t {marked} # 把当前 pane 合并到标记的 pane 所在窗口

move-pane -t {marked} # 移动当前 pane 到标记位置

本质就是 " 剪切板 " 思维：先标记一个 pane（相当于 " 复制 "），再到目标位置执行交换/合并（相当于 " 粘贴 "）。适合跨窗口、跨 session 搬运 pane。

# 2026-04-07 Chatgpt Codex 额度

- plus: 20$/月
- business： 25$/月，现在应该是按 token 计费了
- pro: 200$/月
...
最新额度：  
普号：周限 10.58 美元  
Plus：5 小时 27.67 美元；周限 92.23 美元  
Team：5 小时 16.24 美元；周限 135.33 美元  
Pro：5 小时 152.23 美元；周限 507.43 美元

换算数量差距：  
1 个 Plus=8.71 个普号  
1 个 Team=12.79 个普号  
1 个 Pro=3.75 个 Team=5.5 个 Plus=47.96 个普号

# 2026-04-03 iOS 的 Assets

## 什么是 Assets？

`Assets` 的全称是 **Asset Catalog**（资源目录，文件后缀是 `.xcassets`）。它是 Xcode 提供的一种用于优化、组织和管理应用内**视觉和 UI 资源**的系统功能。

它可以被看作是一个具有 " 智能管理 " 功能的特殊文件夹。它最大的优势在于：

1. **应用瘦身 (App Thinning/Slicing)**：当您的应用发布到 App Store 时，如果不同设备下载，苹果服务器会自动从 Assets 中剥离出该设备不需要的资源文件。比如，大屏幕手机只会拉取 `@3x` 清晰度的图，不会下载 `@1x` 和 `@2x` 的图，从而大大缩小用户下载的 App 体积。
2. **主题适配管理**：您可以非常方便地在这里为一个资源设置不同状态的变体（例如：设置同一个颜色或图片在 " 普通模式 " 和 " 深色模式 " 下分别显示什么样子），代码中只需引用该资源名称即可，系统自动响应切换。

## 资源 " 都 " 应该放到这里吗？

**结论是：视觉相关的 UI 资源绝大部分应该放这里，但并非 " 所有 " 资源都放这里。**

需要根据资源类型来区分：

✅ **强烈建议放进 `Assets` 的资源**：

- **静态图像素材**：所有的 PNG, JPG UI 切图，以及 SVG、PDF 等矢量图。
- **应用图标**：如您所见的 `AppIcon`。
- **颜色定义 (Color Set)**：项目里反复用到的全局主题色，建议像图中的 `AccentColor` 一样定义在这里，方便做深浅色模式适配。
- 特定复杂资源：如 AR 资源、特定的数据资源资产等。

❌ **不应该/通常不放在 `Assets` 里的资源**（这类资源通常会被当做普通文件，直接拖入项目目录树中进行管理）：

- **音视频文件**：`.mp3`, `.wav`, `.mp4` 等。
- **字体文件**：`.ttf`, `.otf`。
- **庞大的静态数据文件**：如用于初始化的大型 `.json`, `.csv` 或 `.sqlite` 数据库文件。（虽然 Assets 支持 "Data Set"，但通常直接放外层用 `Bundle.main.url` 读取更符合常见开发习惯）。
- **UI 布局文件**：`.storyboard`, `.xib`。
- **配置或本地化文本**：`.plist`, `.strings` 文件。

## 服务器存放

1. **服务器存放**：放在您的服务器（如 AWS S3 或阿里云 OSS 结合 CDN 上）。
2. **下载到本地沙盒**：当游戏启动后（通常是玩家点击登录后，出现一个进度条 " 正在检查更新 / 下载资源包..."），您的代码会去服务器对比版本号，将有差异或新增的文件下载到玩家手机的 **iOS 沙盒文件系统**中（通常是 `Documents` 或 `Library/Caches` 目录下）。
3. **读取方式不同**：
	- 读取 `Assets` 里的图片使用：`Image("LoginBackground")` (SwiftUI) 或 `UIImage(named: "LoginBackground")` (UIKit)。
	- 读取动态下载的图片，则是通过它的绝对文件路径：`Image(uiImage: UIImage(contentsOfFile: sandboxPath))`。

# 2026-03-25 Tmux 点击文件

tmux 开启了 set -g mouse on 时，Cmd+Click 会被 tmux 拦截，需要用 Cmd+Shift+Click 才能打开链接

# 2026-03-25 Yazi 文件导航

```bash
搜索能力：
文件名搜索：按 f 过滤当前目录，按 / 搜索（默认用 fd）
文件内容搜索：按 S（大写），调用 rg（ripgrep）搜索内容，结果可直接打开
```

# 2026-03-25 Ghostty 查看图片
...
# 2026-03-24 Git Revert Merge

```bash
## 核心原因：Merge Commit 有两个父提交
...
普通 commit 只有一个父提交，Git 很清楚 revert 时该"撤销什么、保留什么"，所以直接：

<BASH>

git revert C    # ✅ 不需要 -m，没有歧义
...
main:       A --- B --- M (5d496dc75)  ← 这个 merge commit 有两个父提交

                       /

feature:   X --- Y ---

           (video-template-batch-debug-unblock)

**Merge commit `M` 同时有两个父亲：**

|父提交|含义|-m 参数|
|---|---|---|
|**parent 1**|main 分支（被合入的目标分支）|`-m 1`|
|**parent 2**|feature 分支（合入的来源分支）|`-m 2`|

你可以验证：

<BASH>

git cat-file -p 5d496dc75

# 会看到：
# parent xxxxx   ← parent 1 (main)
# parent xxxxx   ← parent 2 (feature)

> **出现的顺序就是编号！** 第一行 parent = 1，第二行 parent = 2。就这么简单。


### Git 的困惑

当你执行 `git revert 5d496dc75` 时，Git 会问：

> "你要撤销到哪个父提交的状态？是保留 main 的代码，还是保留 feature 的代码？"

<BASH>

$ git revert 5d496dc75

error: commit 5d496dc75 is a merge but no -m option was given.
...
# ✅ -m 1：保留 main 的代码，撤销 feature 带来的变更

git revert -m 1 5d496dc75

# 效果：回到合并前 main 分支的状态（这就是你要的回滚）

# ❌ -m 2：保留 feature 的代码，撤销 main 上的东西

git revert -m 2 5d496dc75

# 效果：反而只保留了有 bug 的代码（通常不是你想要的）

---

### 一句话总结

> **普通 commit 有 1 个父提交，Git 知道撤销什么；Merge commit 有 2 个父提交，你必须用 `-m` 告诉 Git 保留哪一边。`-m 1` = 保留 main，撤销合入的 feature 代码。**
...
# 2026-03-04 obra/superpowers 是什么

**核心定位**：这是一个为 **Claude Code（AI 编程 CLI 工具）** 设计的技能（Skills）插件框架，而非一个应用代码库。

它的工作方式：

- 安装后，在 Claude Code 每次会话启动时，通过 **hook 机制**自动注入一组 " 技能元信息 "
- Claude 在执行任何任务前，会自动搜索并加载相关技能（如 TDD、头脑风暴、计划制定）
- 技能是**强制性工作流**，不是建议——Claude 必须遵循

核心技能示例：

| 技能                            | 作用                |
| ----------------------------- | ----------------- |
| `brainstorming`               | 写代码前先澄清需求、探索方案    |
| `test-driven-development`     | 强制红绿重构循环，先写测试再写实现 |
| `write-plan` / `execute-plan` | 先出实现计划再动手         |

安装：Fetch and follow instructions from <https://raw.githubusercontent.com/obra/superpowers/refs/heads/main/.codex/INSTALL.md>

# 2026-02-25 Mongodb 云端连接

## MongoDB Atlas 集群部署在云端，你的 ECS 任务要连接它，有两条路

| 方案                            | 类比             | 说明                                                                  |
| ----------------------------- | -------------- | ------------------------------------------------------------------- |
| **公网 + IP 白名单**               | 从大门进，靠门卫认人（IP） | ECS → 经过一个有固定公网 IP 的出口（NAT Gateway）→ 走公网 → 到 Atlas。把这个固定 IP 加到白名单即可 |
| **VPC Peering / PrivateLink** | 开一条内部通道，不走大门   | 你的 AWS VPC 和 Atlas 的 VPC 之间建立专用隧道，流量不经过公网，安全性最高                     |

## 公有子网 Vs 私有子网

| 概念       | 类比           | 特征                                    |
| -------- | ------------ | ------------------------------------- |
| **公有子网** | 沿街的店铺，直接面对马路 | 子网里的资源可以直接获得公网 IP，直接访问互联网             |
| **私有子网** | 在大楼内部，不直接对外  | 子网里的资源**没有**公网 IP，要上网必须经过 NAT Gateway |

- 打开 AWS Console → ECS → 你的 Cluster → 你的 Service → 看 **Network Configuration**
- 里面会显示 Subnet ID，点进去看它的路由表：
	- 如果路由表有 `0.0.0.0/0 → igw-xxx`（Internet Gateway）→ 是**公有子网**
	- 如果路由表有 `0.0.0.0/0 → nat-xxx`（NAT Gateway）→ 是**私有子网**
	- 如果没有 `0.0.0.0/0` 的路由 → 那就没有出网能力

## NAT Gateway

类比：大楼的统一出口。大楼里（私有子网）的人要出去办事，都得从这个统一出口走，外面的人只看到这个出口的地址（固定公网 IP），看不到里面谁在出去。

关键点：NAT Gateway 有固定的公网 IP（Elastic IP），可以用来做 MongoDB Atlas 的 IP 白名单。

# 2026-02-25 ECS 部署

## **ECS Cluster

**ECS Cluster 是 " 资源池 + 逻辑分组 " 的概念**，通常按以下维度组织：

| 维度   | 示例                           |
| ---- | ---------------------------- |
| 按产品线 | IfOnlyService、AgentService   |
| 按环境  | prod-cluster、staging-cluster |
| 按团队  | team-a-cluster               |

```bash
Cluster（集群）
  └── Service（服务：维持几个 Task 运行、关联 ALB）
        └── Task Definition（任务定义：用哪个镜像、多少 CPU/内存、环境变量）
...
```bash
EC2 宿主机（公寓）IP: 10.0.1.100
├── 容器A（租客A）→ 没有自己的门牌号，通过公寓大门的不同房间号访问
│   宿主机:32768 → 容器:8848
├── 容器B（租客B）
│   宿主机:32769 → 容器:3000
```

- 容器没有独立 IP，靠宿主机的**随机端口**转发
- ALB 需要追踪动态端口，配置复杂
...
├── 容器A → 自己的 ENI，独立 IP: 10.0.1.201:8848
├── 容器B → 自己的 ENI，独立 IP: 10.0.1.202:3000
```

- 每个容器有**自己的私有 IP 和网卡**
- ALB 直接按 IP:Port 路由，简单直接
- 安全组直接绑到容器级别

**结论**：`awsvpc` 更现代、更简单。你已有服务也是 `awsvpc`，保持一致就对了。

## Target Type 四个选项的含义

| Target Type                   | 含义                 | 适用场景                                         |
| ----------------------------- | ------------------ | -------------------------------------------- |
| **Instances**                 | 按 EC2 实例 ID + 端口路由 | `bridge` / `host` 网络模式，ALB 把流量发给 EC2 实例，再转发给容器 |
| **IP addresses**              | 按 IP 地址 + 端口路由     | `awsvpc` 模式，ALB 直接把流量发给容器的 IP                |
| **Lambda function**           | 路由到 Lambda 函数      | 无服务器场景                                       |
| **Application Load Balancer** | 路由到另一个 ALB         | 多层负载均衡                                       |

**为什么 awsvpc 必须选 IP？**
`awsvpc` 模式下，每个 Task（容器）有**自己的独立网卡（ENI）和私有 IP**，而不是挂在 EC2 实例上。所以 ALB 需要直接按 IP 找到容器，而不是按 EC2 实例 ID。
如果选了 `Instances`，ALB 只认 EC2 实例 ID，它不知道容器的独立 IP，流量就送不到。

## Subnet 是什么

**子网 = VPC 里的一个网段**，决定容器放在哪个网络区域。

```bash
VPC (10.228.0.0/16)
├── 公有子网（有公网路由，Auto-assign public IP = Yes）
│   └── vis-server-public-subnet3-us-west-2c
└── 私有子网（通过 NAT Gateway 访问公网）
    └── vis-server-private-subnet2-us-west-2b
```

## Ecs 自动注册 Target

ECS 自动注册 target 的前提是 **ECS Service 配置了 `loadBalancers`**，将 Service 与 Target Group 关联。你的 `LogicServer` 服务很可能创建时没有绑定 `canvas-logic-server-tg`，所以 ECS 不会自动注册。

去 ECS → LogicServer → **Configuration and networking** tab → 看 **Load balancing** 部分是否为空。

## 证书配置

- ACM 申请证书
- ALB 加 HTTPS Listener，Protocol: HTTPS, Port: 443，SSL certificate: 选刚申请的
- Route53 指向
...
3. 创建 target group
4. 创建 alb
5. 创建 ecs service，选择 target group

# 2026-02-24 Clash Verge 配置

- **IPv6 开关（总开关）**：决定内核是否处理/转发 IPv6 流量（TUN/虚拟网卡场景尤其明显）。
	- 建议关掉
- DNS 覆写：决定 " 你在 Verge UI 里填的 DNS 设置 " 是否覆盖/接管订阅配置里的 dns 段。开了覆写后，很多行为会以 UI 配置为准（不少人因此遇到 hosts/覆盖逻辑相关的坑）。
	- 建议关掉
- DNS 的 IPv6（AAAA）：决定 DNS 是否解析并返回 AAAA 记录；mihomo 文档明确说关掉后会对 AAAA 返回空结果。
	- 建议和外面的 IPv6 开关配套

# 2026-02-09 TUN 模式，数据库连不上

**TUN 接管了系统全局流量后，路由或 DNS/权限/防火墙其中一环出错，导致流量被黑洞**。

[TCP] 198.18.0.1:51627(Navicat Premium) --> 34.208.231.7:22 match IPCIDR(34.208.0.0/12) using Netflix[🇨🇳 台湾标准 IEPL 专线 1]

你连 34.208.231.7:22（SSH）时，被规则命中了一个 IP 段规则（34.208.0.0/12），然后被丢进了你的 Netflix 策略组。

**精确 IP 放行（最稳）**  
`IP-CIDR,34.208.231.7/32,DIRECT,no-resolve`

`no-resolve` 的意思是：**在做 " 目标 IP 类规则 " 的匹配时，不要为了匹配去触发 DNS 解析**。

在 mihomo/Clash 的规则里，像 `IP-CIDR / IP-CIDR6 / GEOIP` 这类属于 "**按目标 IP**匹配 " 的规则。  
当一次连接的目标是 " 域名 "（比如 `example.com:443`）时，内核为了判断 " 这个域名解析出来的 IP 是否落在某个 IP 段里 "，**会先去做一次 DNS 解析**。mihomo 文档明确说明了这一点，并说明 `no-resolve` 可以用来跳过这个解析过程。

# 2026-02-09 路由器 Openclash

## Fake-IP

客户端发起 DNS 请求时，Clash 会立刻返回一个保留地址（例如 198.18.0.0/16 段），同时去上游查真实结果；如果命中代理规则等，就让代理端远程解析，从而减少等待时间、提升体验。

## 接管方式

**本质是 "UDP 接管力度 " 从弱到强：增强 < TUN < 混合/游戏**（越往右越 " 全接管 "，也越可能与一些特殊网络/旁路由/IPv6/局域网场景产生兼容性问题）。
- **Fake-IP（增强）模式**  
	- 特点：主打网页浏览流畅、响应快（DNS 返回 fake-ip 立即响应）。
- Fake-IP + **TUN**
	- 能够代理 " 使用域名的 UDP 链接 "
- Fake-IP + **混合**
	- **能够代理所有 UDP 链接**，提升 NAT 等级，改善游戏联机体验。

## 规则附加

1. 进入 OpenClash → **覆写设置** → **规则设置**
2. 找到 **自定义规则（优先匹配）** 或 **规则附加（前置规则）**
3. 添加你的规则：
	- IP-CIDR,34.208.231.7/32,DIRECT,no-resolve
测试：
4. 点击你截图中的 **Dashboard** 或 **Yacd** 进入控制面板
5. 查看 **规则 (Rules)** 标签页，确认规则已加载且位置靠前
6. 或者在 **日志 (Logs)** 中查看匹配情况

## 私有 IP 10.0.网段走代理

插件设置 ->流量控制 -> 本地 IPv4 绕过地址
...
| 年份   | 系统名（版本）                        | 代表性功能/关键词                                                                                   |
| ---- | ------------------------------ | ------------------------------------------------------------------------------------------- |
| 2001 | Mac OS X 10.0 **Cheetah**      | Aqua 界面、Dock、基于 UNIX 的新架构（OS X 起点）                                                          |
| 2001 | Mac OS X 10.1 **Puma**         | 性能/稳定性大幅改进、补齐关键功能（如 DVD 播放等）                                                                |
| 2002 | Mac OS X 10.2 **Jaguar**       | Quartz Extreme、iChat、Address Book（通讯录）等                                                     |
| 2003 | Mac OS X 10.3 **Panther**      | Exposé、FileVault、Safari（首次成为系统核心浏览器之一）                                                      |
| 2005 | Mac OS X 10.4 **Tiger**        | Spotlight、Dashboard、小组件生态起飞（这一代影响很大）                                                        |
| 2007 | Mac OS X 10.5 **Leopard**      | Time Machine、Spaces、多项 UI/系统能力升级                                                            |
| 2009 | Mac OS X 10.6 **Snow Leopard** | " 以优化为主 " 的一代：性能、稳定性、底层改造                                                                   |
| 2011 | OS X 10.7 **Lion**             | Launchpad、Mission Control、更多向 iOS 学习的交互                                                     |
| 2012 | OS X 10.8 **Mountain Lion**    | 通知中心、iMessage（信息）、Gatekeeper 安全机制                                                           |
| 2013 | OS X 10.9 **Mavericks**        | 省电/内存管理优化、压缩内存等系统级效率提升                                                                      |
| 2014 | OS X 10.10 **Yosemite**        | 全新扁平化设计、Continuity/Handoff（跨设备协作起点）                                                         |
| 2015 | OS X 10.11 **El Capitan**      | System Integrity Protection（SIP）、性能与稳定性优化                                                   |
| 2016 | macOS 10.12 **Sierra**         | 命名转为 "macOS"、Siri 上 Mac、iCloud Drive 深化                                                     |
| 2017 | macOS 10.13 **High Sierra**    | APFS 文件系统、Metal 2 等底层能力加强                                                                   |
| 2018 | macOS 10.14 **Mojave**         | Dark Mode、隐私权限弹窗体系更完善                                                                       |
| 2019 | macOS 10.15 **Catalina**       | 仅支持 64 位应用、Catalyst（iPad App 上 Mac）等                                                        |
| 2020 | macOS 11 **Big Sur**           | 大改版 UI、控制中心风格、Apple Silicon 时代系统形态成型                                                        |
| 2021 | macOS 12 **Monterey**          | Universal Control、快捷指令 Shortcuts 上 Mac 等                                                    |
| 2022 | macOS 13 **Ventura**           | Stage Manager、系统设置改版等（更偏 " 体验与工作流 "）                                                        |
| 2023 | macOS 14 **Sonoma**            | 桌面 Widgets、视频会议增强（Presenter Overlay/Reactions）、Safari Profiles、Game Mode                    |
| 2024 | macOS 15 **Sequoia**           | iPhone Mirroring、全新 Passwords App、Safari 更新等                                                |
| 2025 | macOS **Tahoe 26**             | "Liquid Glass" 新设计语言、Phone App/Live Activities 上 Mac、Spotlight 史上最大更新、Apple Intelligence 扩展 |

你会看到从 macOS 15 直接跳到 **macOS Tahoe 26** 这种 " 更怪 " 的数字——这是 Apple 在 WWDC 宣布的统一命名：iOS / iPadOS / macOS / watchOS 等都用同一个 "26"，对齐生态、降低用户记忆成本。

# 2026-01-29 Cline 和 Roo Code 和 Kilo Code

- Cline 是上游/起点之一：它早期叫 ClaudeDev，后来改名为 Cline（因为不再只围绕 Claude）。
- Roo Code 与 Cline 有明显的分叉渊源：Kilo 团队自己写得很直白——他们 "fork 了 Roo Code，而 Roo Code 本身是 Cline 的 fork"。
- Kilo Code 则更像 " 在 Roo 的基础上继续演进 + 合并上游能力 "：Kilo 官方文章说自己在做 feature-merge：把 Cline、Roo Code 和自家增强合到一起。
一句话：Cline →（分叉）Roo Code →（再分叉/再合并）Kilo Code，并且 Kilo 明确在 " 合并两家优点 + 自己再加料 "。

# 2026-01-27 USB-A 和 USB-C （TODO： 再次更新）

USB-A 和 USB-C 的区别主要在**外形（接口形状）**、**是否可正反插**、以及**常见用途**。另外，"Type-C" 指的就是 **USB-C** 这种接口形状。

## 1) USB-A：传统 " 扁口 "

- 长什么样：长方形、比较 " 扁 " 的口（电脑上最常见那种）。
- 特点
	- 不能正反插（插反了就插不进去）
	- 常见在：台式机/笔记本、充电器、U 盘、鼠标键盘接收器等
- 你听到的 "USB 3.0 蓝色口 " 很多就是 USB-A（但颜色不是硬标准）

## 2) USB-C（Type-C）：小椭圆口

- 长什么样：小一些、两边圆角的 " 椭圆形/胶囊形 " 接口。
- 特点
	- 可以正反插
	- 现在手机、平板、很多笔记本、显示器、扩展坞都用它
	- 它能承载很多协议：充电（PD）、数据、视频（DP Alt Mode / Thunderbolt 取决于设备）——但 " 长得一样 " 不代表功能都一样（有的只能充电/慢速数据，有的能高功率快充 + 高速 + 外接显示器）

## 3) "Type C 是哪一种？"

- Type-C = USB-C（同一种东西的两种叫法）
- 你可能还会看到 USB-A / USB-B / Micro-USB 这种 "Type"，它们都是在说接口外形规格。

## 4) 一个常见误区：口型 ≠ 速度/版本

- USB-A 也可能很快（例如 USB 3.x）
- USB-C 也可能不快（例如只做 USB 2.0 数据线）
- 真正决定能力的是：设备接口实现的协议 + 线材规格（尤其 Type-C 线差别很大）

双公头 usb 线是什么意思：

- **USB-A ↔ USB-A**（两头都是传统扁口插头）
- **USB-C ↔ USB-C**（两头都是 Type-C 插头）
- **USB-A ↔ USB-C**（一头 A 插头，一头 C 插头）——这严格说不是 " 两头一样 "，但仍然是 " 两头都是公头 "。

# 2026-01-16 Resty r.SetDoNotParseResponse(true)

默认情况下，Resty 会在收到响应后，解析（parse）response body，主要做几件事：

- 根据 `Content-Type` 自动解析 JSON、XML 等格式；
- 把解析结果填充进 `SetResult()` 或 `SetError()` 中指定的结构；
- 自动关闭响应流。
...
表示告诉 Resty： 不要自动解析响应体（response body），而是直接把 `*http.Response` 原始响应对象返回给我。

- Resty 不会去 " 消费 "（read）响应体；
- 你需要自己去读取；
- 你拿到的 `resp.RawBody()` 依然是可读的 `io.ReadCloser`；
- 你也需要负责在用完之后关闭它。

实际使用场景举例: 这类设置常见于 下载文件、大数据流处理、或需要自己处理流的情况。

# 2026-01-16 重试风暴和惊群效应

当系统出现一点小故障时，大家都在拼命重试，结果把原本就喘不过气的小故障，瞬间变成了一场压垮整个系统的海啸。

这就是一场 " 自己发起的 DDoS 攻击 "。某个服务挂了，上游服务不信邪疯狂重试，请求量指数级增加，最终把本来还能勉强干活的服务彻底踩死。

原因：

1. 默认配置坑：很多框架默认开启重试。
2. 无脑重试：不管报什么错都重试。
3. 缺乏协调：A 调 B，B 调 C，大家都在重试，乘法效应爆炸。
如何解决：
	- 退避（等一会再试）、抖动（别一起试）、熔断（别试了，歇菜吧）。
	- 限制重试层级，只有最末端服务进行有限的退避重试。

惊群效应 "(Thundering Herd) 不是重试风暴，但很像。

- 惊群：是大家抢资源（比如 100 个进程抢 1 个新连接）。
- 重试风暴：是大家给压力（100 个请求变成 1000 个请求）。
- 冷知识：为了防止重试风暴，Google 的 SRE 团队建议设置一个全局重试预算 (Retry Budget)。比如规定：某个服务每分钟只有 10% 的请求允许是 " 重试请求 "，超过了直接丢弃，不准重试！

# 2026-01-15 Openapi 规范和 Swagger

1. Code-First vs. Design-First
	- 黑话：代码优先 vs. 设计优先。
	- Design-First：先画精细图纸（写 OpenAPI），确认无误后再动工盖房。适合大公司、跨部门协作。
	- Code-First：先盖房子（写代码），盖完再画图纸（生成文档）。*适合快速迭代的小项目。*
2. Mock Server
	- 黑话：模拟服务器。
	- 人话：替身演员。后端还没写完代码，前端急着要调试，就用 Swagger 根据 OpenAPI 规范生成一个假的服务器，给假数据，让前端先干活。
- 文档与代码不同步 (僵尸文档)
	- 误区：代码改了逻辑，忘了更新 Swagger/OpenAPI 文档。
	- 纠正：这是大忌！要么使用 Code-First 自动生成，要么在 CI/CD 流程中加入检测机制，强迫文档更新。

| 维度       | Code-First (代码优先)                               | Design-First (设计优先)                                          |
| -------- | ----------------------------------------------- | ------------------------------------------------------------ |
| **工作流**  | 1. 写 Go/Java 代码 <br> 2. 加注释/注解 <br> 3. 工具自动生成文档 | 1. 写 OpenAPI (YAML) 文档 <br> 2. 评审文档 <br> 3. 工具生成 Go/Java 桩代码 |
| **优点**   | **快！** 开发人员最爽，不用学 YAML 语法，代码即文档，不会不同步。          | **稳！** 前后端并行开发（Mock 数据），接口定义清晰，跨语言协作容易。                      |
| **缺点**   | **脏！** 业务代码里混杂大量注解；文档结构往往不规范；很难描述复杂的多态结构。       | **慢！** 门槛高，需要手写 YAML；前期投入时间多。                                |
| **适用场景** | 内部小项目、单体应用、快速原型开发 (MVP)。                        | 微服务架构、对外公开 API、大型团队协作。                                       |
|          |                                                 |                                                              |

- 2015 年 Swagger 规范捐赠给了 Linux 基金会，改名叫 OpenAPI，但工具还叫 Swagger。
- 后端（编写/生成）、前端（查看/联调）、测试（接口测试）、产品（验收逻辑）。
- 手写 YAML/JSON 文件；在 Java/Go/Python 代码中加注特殊注解（Annotation），启动时自动提取。
- 任何人只要猜到 URL 就能看到你的所有接口甚至直接攻击。务必在生产环境 (Profile: Prod) 禁用 Swagger！
