---
{"aliases":[],"tags":[],"title":"技术卡片笔记","date":"2025-06-01T07:55:23+08:00","date_modify":"2026-01-21T00:32:01+08:00","dg-publish":true,"permalink":"/Publish/技术卡片笔记/","dgPassFrontmatter":true,"created":"2025-06-01T07:55:23+08:00","updated":"2026-01-21T00:32:01+08:00"}
---


# 2026-01-17 MCP Context7

Upstash/Context7 是一个基于 MCP (Model Context Protocol) 协议的开发者文档 RAG（检索增强生成）引擎。它通过动态检索并注入最新的、版本特定的技术文档到 LLM 上下文中，有效解决了大模型训练数据的知识截断（Knowledge Cutoff）问题，确保生成代码的 API 准确性与时效性。

- 实时上下文增强 (Real-time Context Augmentation) 不依赖模型静态权重中的过时知识，实时从官方源获取最新文档与 Code Snippets，消除因 API 废弃或变更导致的模型幻觉 (Hallucinations)。
- 版本感知检索 (Version-Aware Retrieval) 支持语义化的版本锚定（如 `Next.js v14`），通过精准的库 ID 解析（Library ID Resolution），确保生成的代码符合特定版本的语法规范。
- 无缝 IDE 集成 (Native IDE Integration) 作为标准的 MCP Server 运行，能够直接挂载至 Cursor、Trae、Claude Code 等 AI 辅助编程工具。通过自然语言指令（如 `use context7`）即可触发提示词工程 (Prompt Engineering) 优化，自动注入参考资料。

适用场景

- 使用快速迭代的框架时：比如 Next.js、Supabase、Cloudflare Workers 等更新频繁的技术栈。
- 避免 API 报错：当你发现 AI 写的代码总是报错 " 方法不存在 " 时，使用 Context7 可以获取正确的 API 用法。
- 新库学习：让 AI 快速掌握一个它从未在训练数据中见过的全新开源库。
...
# 2026-01-15 Docker-compose 和 Docker Compose

1. `docker-compose` (V1) 的状态:
	- 已停止维护 (End of Life)。
	- Docker 官方在 2023 年 6 月之后已经不再支持 V1 版本，不再提供安全更新或 bug 修复。
2. `docker compose` (V2) 的状态:
	- 当前标准。
	- 所有新的功能（如对 Apple M1/M2 芯片的更好支持、GPU 支持改进等）都在 V2 中开发。

为什么 Docker 官方要从 V1 切换到 V2？除了集成度，还有底层语言的改变：

1. 编程语言:
	- `docker-compose` (V1) 是由 Python 编写的。
	- `docker compose` (V2) 是由 Golang (Go 语言) 编写的。
2. 性能与体验:
	- 由于 Docker 引擎本身就是用 Go 写的，V2 版本（Go）能更好地与 Docker 核心代码集成。
	- V2 启动速度更快，并发执行效率更高，且能直接复用 Docker CLI 的上下文（Context）和认证机制，体验更加顺滑。
...
在中国传统文化中，完整的兄弟排行顺序通常是四个字：" 伯、仲、叔、季 "。

1. 伯 (bó)：代表老大（长子）。
	- " 白 " 在古文中往往带有 " 大 "、" 首领 " 或者 " 白发（年长）" 的含义。
	- 用法： 父亲的哥哥称为 " 伯父 "。
	- 注：有时 " 孟 " 也指老大，但 " 孟 " 通常指庶长子（侧室所生），" 伯 " 指嫡长子（正妻所生），或者在现代混用中，" 伯 " 更常用于称呼。
	- 孟：更强调 " 初始 "、" 庶长 "（非正妻生的老大）或单纯的排序第一。
2. 仲 (zhòng)：代表老二（次子）。
	- 这个字最直观，就是 " 中间的那个人 "。
	- 它在 " 伯 " 和 " 叔 " 之间起连接作用。比如孔子字 " 仲尼 "，因为他排行老二。
3. 叔 (shū)：代表老三（或者泛指比老大老二小的弟弟）。
	- 左边是 " 尗 "（shú，豆类的总称），右边是 " 又 "（手，表示动作）。
	- 本义是捡拾豆子，古人认为 " 捡豆子 " 这种活多由年轻、年幼的人去干。
	- 用法： 父亲的弟弟称为 " 叔叔 "。
	- 在只有兄弟三人的情况下，" 叔 " 就是老三。在兄弟众多的情况下，" 叔 " 可以指代中间的弟弟们。
4. 季 (jì)：代表最小（老幺）。
	- 上面是 " 禾 "（庄稼），下面是 " 子 "（小孩/种子）。指幼禾，即最晚的一茬庄稼，或者庄稼生长的末期。
	- 用法： 指最小的弟弟。
	- 这个字也延伸到季节的划分，如 " 孟春 "（春季第一个月）、" 仲春 "（春季第二个月）、" 季春 "（春季最后一个月）。

# 2026-01-10 Ralph Loop 拉尔夫循环

许多人以为 AI 编程只需把代码发给 GPT。但真正的威力在于 " 循环（Loop）"。 Ralph 这类工具的核心价值，不在于 Claude 模型本身有多强，而在于它实现了 " 写代码 -> 运行 -> 报错 -> 把报错贴回去 -> AI 根据报错修正代码 " 的自动化闭环。这模仿了人类程序员 Debug 的真实过程。

- Ralph 遇到一个顽固的 Bug，反复修改反复错，你的信用卡在燃烧。
- 设置预算阈值。监控 Token 使用量，如果看到它在同一个问题上卡了超过 3 次，立刻 Ctrl+C 强行打断，人工介入。
...
这样 Agent 就可以在不需要你点击 " 允许 " 的情况下，自主运行数小时。启动循环：	在终端中输入你的指令。

# 2026-01-10 Opencode Agents

<https://opencode.ai/docs/agents/>

OpenCode 内置了两个主代理和两个子代理。

- Build 是**默认的**主代理，启用了所有工具。这是开发工作的标准代理，适用于需要完全访问文件操作和系统命令的情况。
- Plan 这是一个专为规划和分析而设计的受限代理。我们采用权限系统，让您拥有更多控制权并防止意外更改。

子代理

- General： 一款通用型智能代理，可用于研究复杂问题、搜索代码和执行多步骤任务。当您搜索关键词或文件，但又不确定能否在最初几次尝试中找到正确结果时，可以使用它。
- Explore：一款专为探索代码库而设计的快速代理。当您需要按模式快速查找文件、搜索代码中的关键字或解答有关代码库的问题时，可以使用它。

使用：

1. 对于主代理，会话期间可以使用 **Tab** 键切换代理。您也可以使用已配置的 `switch_agent` 快捷键。
2. 子代理由主要代理根据其描述**自动**执行专门任务，或者在消息中 **@提及**子代理人即可

# 2026-01-10 Subagent

- Agent（主智能体） 就是行政总厨（Head Chef）。他统筹全局，负责接单、安排工作、最后摆盘出餐，他什么都懂一点，但不可能同时又切菜又烤肉又做甜点。
- Sub-agents（子智能体） 就是专门负责各个档口的厨师。比如 " 甜点师 "、" 烤肉师 "、" 凉菜师 "。他们只精通自己的领域，接到主厨的命令干活，干完把成品交给主厨。

子代理是 Claude Code 可以委派任务的预配置 AI 人格。每个子代理：
...
- 每个子代理在自己的上下文中运行，防止主对话被污染，并使其专注于高级目标。
- 子代理可以使用针对特定领域的详细说明进行微调，从而在指定任务上获得更高的成功率。
- 创建后，您可以在不同项目中使用子代理，并与您的团队共享它们以实现一致的工作流。
- 每个子代理可以具有不同的工具访问级别，允许您将强大的工具限制为特定的子代理类型。

## Subagent 和 Skills

Subagent (子代理)

子代理是一个专门化的 AI 助手，设计用于处理特定领域的复杂任务。

- **调用链**：`用户输入/@引用` -> `TaskTool` -> `Session.create()` (新会话) -> `SessionPrompt.prompt()` (子代理运行)。
- **逻辑**：当主代理（如 `build`）决定调用子代理时，系统会创建一个全新的会话。子代理在这个会话中可以进行成百上千次的工具调用（如深度的代码搜索），而不会消耗主对话的 Token，也不会产生干扰。

Skills (技能)

技能是按需加载的专家知识库。

- **调用链**：`代理发现需求` -> `SkillTool` -> `ConfigMarkdown.parse()` (读取文件) -> `注入当前对话`。
- **逻辑**：代理通过 `skill` 工具列出所有可用技能及其描述。当任务匹配某个技能的描述时，代理主动加载该技能的 `SKILL.md` 内容。这些内容通常是具体的步骤指南、API 规范或最佳实践。

使用 Subagent 的场景

- **需要隔离性**：任务会产生大量中间输出（如 `@explore` 遍历文件系统）。
- **需要不同的模型或参数**：例如使用更廉价的模型进行初步搜索，或调低温度以获得更确定的结果。
- **需要安全边界**：子代理可以被赋予比主代理更严格或更宽松的权限。
- **多任务并行**：可以同时启动多个子代理执行不同任务。

使用 Skills 的场景

- **需要标准化流程**：例如公司内部的 git 提交规范、发布流程指南。
- **共享领域知识**：例如特定框架的 API 文档、复杂算法的实现参考。
- **轻量级引导**：只需几行指令就能让代理表现更好，不需要独立的会话。
- **指令复用**：希望在 `build`、`plan` 或任何自定义代理中都能一键调用的能力。
