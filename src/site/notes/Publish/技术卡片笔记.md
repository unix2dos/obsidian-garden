---
{"aliases":[],"tags":[],"title":"技术卡片笔记","date":"2025-06-01T07:55:23+08:00","date_modify":"2025-11-20T01:42:18+08:00","dg-publish":true,"permalink":"/Publish/技术卡片笔记/","dgPassFrontmatter":true,"created":"2025-06-01T07:55:23+08:00","updated":"2025-11-20T01:42:18+08:00"}
---


# 2025-11-19 Mac 破解和软件推荐
...
- 粘贴板
	- oneclip
- 录屏
	- ScreenSage Pro，可以录制窗口，录制自己
- 显示器控制
	- betterdisplay
- 刘海
	- notchnook

# 2025-11-15 电视 Youtube

 YouTube for Fire TV

<https://www.wukongsch.com/blog/zh/10-must-watch-chinese-learning-channels-for-overseas-children-post-16368/>
...
|         |                                                     |
| ------- | --------------------------------------------------- |
| **硬解码** | 使用 GPU（显卡）或专用视频解码芯片来完成视频解码工作。CPU 只负责调度，主要计算由显卡硬件完成。 |
| **软解码** | 使用 CPU 来进行视频解码。所有视频编码格式的计算工作都由 CPU 处理。              |

| 项目        | 硬解码                                         | 软解码                         |
| --------- | ------------------------------------------- | --------------------------- |
| **性能负载**  | 显卡处理，CPU 占用率低，整体系统更轻松。                      | 全靠 CPU，CPU 占用高，解码复杂视频时可能卡顿。 |
| **功耗与发热** | 低功耗、低发热。                                    | 高功耗、高发热。                    |
| **兼容性**   | 显卡需支持该视频格式（如 H.264、H.265、AV1 等）。不支持的格式无法硬解。 | 几乎支持所有视频格式，因为靠软件算法。         |
| **图像质量**  | 一般为标准质量，无法调太多参数。                            | 可通过软件算法优化（如增强滤镜、画质修复等）。     |
| **适用场景**  | 高分辨率、高码率视频（如 4K、8K）。                        | 老旧 CPU 无硬件支持时，或需要特效处理时。     |

# 2025-11-08 4K 和 1080p

| 名称             | 常见称呼          | 代表的分辨率（水平×垂直像素） | 常见用途             |
| -------------- | ------------- | --------------- | ---------------- |
| **720p**       | 高清（HD）        | 1280 × 720      | 网络视频、老电视         |
| **1080p**      | 全高清（Full HD）  | 1920 × 1080     | 普通电脑屏、主流电视       |
| **2160p / 4K** | 超高清（Ultra HD） | 3840 × 2160     | 4K 电视、蓝光电影、高端显示器 |

 **"4K" 和 "2160p" 实际上指的是同一个等级的分辨率。**
- 家用电视/视频标准中，4K=3840×2160。
- 电影制作领域中，4K DCI 标准是 4096×2160，稍宽一点。

| 对比项    | 1080p   | 4K / 2160p        |
| ------ | ------- | ----------------- |
| 水平像素数  | 1920    | 3840              |
| 垂直像素数  | 1080    | 2160              |
| 总像素量   | 约 207 万 | 约 830 万           |
| 像素数量倍数 | —       | 是 1080p 的约 4 倍    |
| 清晰度    | 已经很清晰   | 更细腻、放大后仍锐利        |
| 文件体积   | 较小      | 大约是 1080p 的 2~4 倍 |

 **4K（2160p）的画面像素点是 1080p 的 4 倍，看上去更细、更清、更真实。
