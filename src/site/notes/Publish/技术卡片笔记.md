---
{"aliases":[],"tags":[],"title":"技术卡片笔记","date":"2025-06-01T07:55:23+08:00","date_modify":"2025-07-20T01:46:59+08:00","dg-publish":true,"permalink":"/Publish/技术卡片笔记/","dgPassFrontmatter":true,"created":"2025-06-01T07:55:23+08:00","updated":"2025-07-20T01:46:59+08:00"}
---
...
贴图：截图后点击贴图按钮。
多窗口：按下截图按钮，再按 shift，点多个窗口，然后松开快捷键。e

# 2025-07-19 Obsidian git 自动生成 ai commit

- Commit message on manual commit 和 Commit message on auto commit-and-sync 置为空
- 脚本放到 Commit message script

```bash
git diff HEAD -- ':!library/.obsidian/plugins' | /opt/anaconda3/bin/sgpt '根据更改生成 git 提交信息：要求：1. 遵循 git 提交最佳实践, 重要功能或修复可加类型前缀（如 feat:、fix:） 2. 使用中文 3. 每条信息前加序号 4.直接输出，无需解释'
```
...
# 2025-07-13 Tmux 分屏的技巧

- `prefix ` + ` - ` 水平分屏
- `prefix ` + ` shift` + `-` 垂直分屏
- `prefix` + `z`：暂时将当前窗格全屏/恢复，非常实用！
- `prefix` + `space`：快速切换横竖屏，多个窗口可以切换很多模式。
- `prefix` + `{` `}`: 将当前窗格与上一个窗格交换位置交换位置。
- `oh-my-tmux` 做了很好的优化，你可以按住 `<prefix>` 不松开，然后直接用方向键来调整窗格大小。
- `prefix` + `q`：每个窗格上都会浮现一个巨大的数字。
- `<prefix>` + `+` 让当前 panel 成为 window, 注意再一次还能回到 panel
...
tmux 本身不能保证 SSH 连接不断，但它可以在 SSH 断开后保持会话继续运行。
tmux 保证的是“里子”（工作内容），SSH Keep-Alive 保证的是“面子”（连接不掉线）。

## Tmate 分享终端

tmate 是一个基于 tmux 的终端共享工具，可以让你轻松地与他人共享你的终端会话。它通过 SSH 连接实现安全的终端共享。
...
web session read only: https://tmate.io/t/ro-J9L46PMxLZbEKpbWEC74GScdc
ssh session read only: ssh ro-J9L46PMxLZbEKpbWEC74GScdc@sfo2.tmate.io
# 1. 给别人演示
web session: https://tmate.io/t/HeepQSkVAdcsm4cGGBwrtZraw
# 2. 别人访问可以控制
ssh session: ssh HeepQSkVAdcsm4cGGBwrtZraw@sfo2.tmate.io
```

# 2025-07-02 Vscode 换行

选中文本：Option + Z (Mac)
或者设置里 -> 搜索 word wrap 开启或关闭
